 ...
 */
function string(length, characters = [...numbers, ...uppercaseLetters, ...lowercaseLetters]) {
    let res = '';
...
function vector2D(minX = 0, maxX = 10, minY = 0, maxY = 10) {
    return new Vector2D(number(maxX, minX), number(maxY, minY));
}
function vector3D(minX = 0, maxX = 10, minY = 0, maxY = 10, minZ = 0, maxZ = 10) {
    return new Vector3D(number(maxX, minX), number(maxY, minY), number(maxZ, minZ));
}
/**
 * @param vec the vector that should be randomized
 * @param radius randomizing (0, 0) with radius 5 => min: (-5, -5), max: (5, 5)
 */
function randomizeVector2D(vec, radius = 5) {
    vec.add(vector2D(vec.x - radius, vec.x + radius, vec.y - radius, vec.y + radius));
}
/**
 * @param vec the vector that should be randomized
 * @param radius randomizing (0, 0, 0) with radius 5 => min: (-5, -5, -5), max: (5, 5, 5)
 */
function randomizeVector3D(vec, radius = 5) {
    vec.add(vector3D(vec.x - radius, vec.x + radius, vec.y - radius, vec.y + radius, vec.z - radius, vec.z + radius));
}
/**
